import { UserInfo, WebUserInfo } from '@types'
...
export const webLogin = async (userInfo: UserInfo): Promise<WebUserInfo> => {
  ...
  if (result.status !== 'success') throw new Error(JSON.stringify(result.data))
  ...
  console.log(`[Web Login]: ${userInfo?.username} 已获取token`)
  return {
