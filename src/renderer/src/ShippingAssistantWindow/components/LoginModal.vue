<template>
  ...
  >
    <div class="login-container">
      ...
      <div class="user-list">
        <p class="user-list-title">已保存账号</p>
        <div v-if="(accountList?.length ?? 0) === 0" class="user-list-empty">暂无已登录账号</div>
        <div v-else class="user-list-rows">
          <div
            v-for="account in accountList"
            :key="account.username"
            class="user-list-row"
            :class="{ 'is-account-disabled': account.disable === true }"
          >
            <Checkbox
    ...
    <template #footer>
      ...
      <button class="btn btn-login" :disabled="isLoading" @click="handleLogin">
...
const emit = defineEmits<{
  (e: 'close'): void
  (e: 'login-success', data: WebUserInfo): void
  (e: 'update-account-disable', payload: { username: string; disable: boolean }): void
}>()

const onAccountDisableChange = (username: string, disable: boolean) => {
  emit('update-account-disable', { username, disable })
}
...
const selectedAccount = ref<WebUserInfo>()
...
// 删除账号
const handleDeleteAccount = async (userInfo: UserInfo | WebUserInfo) => {
...
const handleLogin = async () => {
  ...
  try {
    const result = await ipcEmitter.invoke(
      'web-login',
      ipcArg({ username, password, remark: remark }),
    )
...
// 选择已保存的账号
const selectSavedAccount = (userInfo: WebUserInfo) => {
